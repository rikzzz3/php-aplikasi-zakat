...
<script src="https://code.jquery.com/jquery-3.7.1.min.js" integrity="sha256-/JqT3SQfawRcv/BIHPThkBvs0OEvtFFmqPF/lYI/Cxo=" crossorigin="anonymous"></script>
    ...
    <div class="box">
        <h1>Zakat</h1>
        <h4>
            <small>Edit Data Pembayaran Zakat</small>
        ...
        <div class="row">
            <div class="col-lg-6 col-lg-offset-3">
                <?php
                $id = @$_GET['id'];
                $sql_zakat = mysqli_query($con, "SELECT * FROM tb_zakat WHERE id_zakat = '$id'") or die (mysqli_error($con));
                $data = mysqli_fetch_array($sql_zakat);
                ?>
                <form action="proses.php" method="post">
                    <div class="form-group">
                        <label for="nama">Tanggal Transaksi</label>
                        <input type="hidden" name="id" value="<?=$data['id_zakat']?>">
                        <input type="date" name="tgl" id="tgl" value="<?=$data['tanggal_transaksi']?>" class="form-control" required autofocus>
                    <div class="form-group">
                        <label for="nama">Nama</label>
                        <input type="text" name="nama" id="nama" value="<?=$data['nama_muzakki']?>" class="form-control" required>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="tagihan" id="tagihan" value="<?=$data['tagihan_zakat']?>" class="form-control" required readonly>
                    </div>
                    <div class="form-group">
                        <label for="nama">Uang yang Dibayarkan</label>
                        <input type="text" name="pembayaran" id="pembayaran" value="<?=$data['jumlah_uang']?>" class="form-control" required oninput="calculateKembalian()">
                    </div>
                    <div class="form-group">
                        <label for="nama">Kembalian</label>
                        <input type="text" name="kembalian" id="kembalian" value="<?=$data['kembalian']?>" class="form-control" required readonly>
                    </div>
